        ...
        <div className={s.nav}>
            {pages.map(p => {
                return <span onClick={(e) => { props.onPageChange(p) }}>
                    {(p <= props.currentPage + 3 && p >= props.currentPage - 3) && <button className={(p === props.currentPage) ? s.active : s.passive}>{p}</button>}
                    </span>
            })}
        </div>
        {
            props.users.map(u => <div key={u.id}>
                <div className={s.box}>
                    ...
                    <div >
                        {u.followed ? <button disabled={props.following.some(id=>id===u.id)} className={s.button + ' ' + s.unfollow} onClick={() => {
                           props.unfollowTC(u.id);
                        }}>UNFOLLOW</button > : <button disabled={props.following.some(id=>id===u.id)} className={s.button + ' ' + s.follow} onClick={() => {
                            props.followTC(u.id);
                        }}>FOLLOW</button>}
                    </div>
                </div>
